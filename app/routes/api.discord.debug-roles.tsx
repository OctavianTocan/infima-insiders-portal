...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  try {
    ...
    if (!memberResponse.ok) {
      return new Response(
        JSON.stringify({ error: "User not found in guild" }),
        {
          status: 404,
          headers: { "Content-Type": "application/json" },
        }
    ...
    if (!isMemberData(memberRaw)) {
      return new Response(
        JSON.stringify({ error: "Invalid member data from Discord API" }),
        {
          status: 502,
          headers: { "Content-Type": "application/json" },
        }
    ...
    if (!isGuildRoleArray(rolesRaw)) {
      return new Response(
        JSON.stringify({ error: "Invalid roles data from Discord API" }),
        {
          status: 502,
          headers: { "Content-Type": "application/json" },
        }
    ...
    const roleMap = Object.fromEntries(
      allRoles.map((role) => [role.id, role.name])
    );
